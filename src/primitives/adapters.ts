import type { AgentEvidence, BorrowingBase, CdmDeal, EligibilityResult, WaterfallLine } from '../domain/cdm.js';

export function extractDealTerms(deal: CdmDeal): AgentEvidence {
...
export function doubleCheckLoanTape(deal: CdmDeal): AgentEvidence {
...
export function calculateBorrowingBase(deal: CdmDeal): BorrowingBase {
  const results: EligibilityResult[] = deal.loans.map(loan => {
    const reasons = [
  ...
  const balancesByState = deal.loans.reduce<Record<string, number>>((acc, loan) => ({ ...acc, [loan.state]: (acc[loan.state] ?? 0) + loan.currentBalance }), {});
...
export function runAbsBoxWaterfall(deal: CdmDeal, base: BorrowingBase): WaterfallLine[] {
...
export function initiateBankTransfer(waterfall: WaterfallLine[]): string {
